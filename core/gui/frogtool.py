...
def process_sys(drive, system, test_mode):
    ...
    write_index_file(name_map_files, sort_without_file_ext,
                     index_path_files, test_mode)
    write_index_file(name_map_cn, sort_normal, index_path_cn, test_mode)
    write_index_file(name_map_pinyin, sort_normal,
                     index_path_pinyin, test_mode)
...
def convert_zip_image_pairs_to_zxx(roms_path, system):
    ...
    if not img_files or not zip_files:
        return
    print(
        f"Found image and zip files, looking for matches to combine to {sys_zxx_ext}")
    ...
    for img_file in img_files:
        ...
        if not zip_file:
            continue
        converted = convert_zip_image_to_zxx(
            roms_path, img_file, zip_file, sys_zxx_ext)
        if not converted:
    ...
    if imgs_processed:
        print(
            f"Combined {imgs_processed} zip + image pairs into .{sys_zxx_ext} files")
...
def convert_zip_image_to_zxx(path, img_file, zip_file, zxx_ext):
    ...
    try:
        ...
        print(
            f"! Failed deleting source file(s) after creating {zxx_file_name}")
        return False
...
def rgb565_convert(src_filename, dest_filename, dest_size=None):
    ...
    try:
        ...
        print(
            f"! Failed opening destination file {dest_filename} for conversion")
        return False
    ...
    for h in range(image_height):
        for w in range(image_width):
            ...
            if not type(pixel) is tuple:
                print(
                    f"! Unexpected pixel type at {w}x{h} from {src_filename}")
                return False
            r = pixel[0] >> 3
            g = pixel[1] >> 2
            b = pixel[2] >> 3
...
def write_index_file(name_map, sort_func, index_path, test_mode):
    ...
    sorted_pointers = map(
        lambda name: pointers_by_name[name], sorted_display_names)
